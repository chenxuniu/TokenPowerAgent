...
and system telemetry, including Ray-based multi-node runs
\cite{niu2026tokenpowerbench}.  \system{} does not replace that measurement
...
disaggregation, and power policy further alter the realized schedule
\cite{kwon2023vllm,agrawal2024sarathi,zhong2024distserve,lee2026beam}.
Likewise, a parallelism choice that is efficient within one node may incur
collective or pipeline overhead across nodes.  Prior energy studies therefore
find no universally best model--engine--workload configuration
\cite{niu2025engines,fernandez2025energy}.  A useful search state must preserve
workload geometry, serving schedule, topology, and SLO, rather than reduce a
...
measure device-local prefill, decode, and power response directly.  Neither
source observes every scale-dependent term.  Intra-node tensor parallelism
introduces PCIe, NVLink, or NVSwitch collectives; multi-node execution adds
fabric transfer, placement, synchronization, and pipeline gaps; request traces
add queueing and scheduler interference.  Prediction uncertainty must therefore
grow when a candidate crosses an unmeasured hardware, topology, or scale
boundary.

\textbf{Requirement R2: explicit evidence contracts.}  The sandbox must be a
calibration ladder, not a claim that one GPU emulates a cluster.  Every outcome
must retain its fidelity, provenance, and uncertainty, and target-scale
deployment claims must be supported by target-scale measurements.
...
clearly infeasible or dominated does not need a precise target-scale label.
...
calibrates a previously unseen topology.  Generic predictive variance is also
