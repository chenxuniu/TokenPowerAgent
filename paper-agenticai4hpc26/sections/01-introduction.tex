...
\ConfigurationMeasuredRuns{} balanced Probe/Verify measurements expose the
boundary the system must manage: the Sandbox ranks energy well but fails for
TTFT and the Pareto decision; the Probe recovers the true frontier with
\LoneParetoRecall{} recall; and Verify removes two false positives.  Corrected
replay compares four policies across five GPU-hour budgets, while two frozen
...
\begin{itemize}
    ...
    compares four acquisition policies across \BudgetLevels{} budgets without
    spending additional GPU-hours, and a disjoint planner holdout quantifies
    raw proposal accuracy, guard intervention, and end-to-end planner latency.
\end{itemize}
