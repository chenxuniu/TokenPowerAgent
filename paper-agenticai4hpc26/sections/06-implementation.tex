...
constrained planner calls an OpenAI-compatible endpoint and accepts only a JSON
subgoal and rationale.  Schema failure invokes a deterministic fallback; a
separate state-priority guard compares every valid proposal with the required
subgoal and records any replacement.  Candidate selection and every
safety-critical state transition remain outside the language model.
...
\WHILE{$B_s>0$}
  \STATE $(\widetilde g_t,g_t)\gets\textsc{ProposeAndGuard}(S,\mathcal T,\mathcal D,B_s)$
